...
// insert AITS subjective type questions in the AITS subjective type questions table by making a post request
router.post('/insert-aits-subjective-choice-question', (req, res) => {
    ...
    if(!subject_id){
      ...
      let sql = "INSERT INTO aits_subjective_type_question (subject_id, topic_id, question_image_url, correct_answer, total_marks, question_description, solution_image_url, negative_mark, partial_mark) VALUES ?"
...
// Fetch the entire data of the AITS subjective type questions table
router.get('/fetch-aits-subjective-type-questions', (req, res) => {
    let sql = "SELECT * FROM aits_subjective_type_question"
    mysqlConnection.query(sql , (err, result) => {
        if(err){
            res.status(202).send({ error: err })
        }
...
// Fetch a particular id from the AITS subjective type question table using its question id
router.get('/fetch-aits-subjective-type-question-by-questionid/:id', function(req, res) {
    var id = req.params.id;
    var sql = "SELECT * FROM aits_subjective_type_question WHERE question_id="  + mysql.escape(id) ;
    mysqlConnection.query(sql , (err, result) => {
        if(err){
            res.status(202).send({ error: err })
        }
  ...
  // Fetch a particular id from the AITS subjective type question table using its subject_id
  router.get('/fetch-subjective-type-question-by-subjectid/:id', function(req, res) {
    var id = req.params.id;
    var sql = "SELECT * FROM aits_subjective_type_question WHERE subject_id="  + mysql.escape(id) ;
    mysqlConnection.query(sql , (err, result) => {
        if(err){
            res.status(202).send({ error: err })
        }
...
// Fetch a particular id from the AITS subjective type question table using its topic_id
router.get('/fetch-subjective-type-question-by-topicid/:id', function(req, res) {
    var id = req.params.id;
    var sql = "SELECT * FROM aits_subjective_type_question WHERE topic_id="  + mysql.escape(id) ;
    mysqlConnection.query(sql , (err, result) => {
        if(err){
            res.status(202).send({ error: err })
        }
...
// update a particular AITS subjective type questions from the AITS subjective type questions table
router.post('/update-aits-subjective-type-question/:id', function(req, res) {
    var id = req.params.id;
    var sql = "SELECT * FROM aits_subjective_type_question WHERE question_id="  + mysql.escape(id);
    mysqlConnection.query(sql, function(err, result) {
      if(err) {
        ...
        if(result.length !=0){
            ...
            let sql2 = "UPDATE aits_subjective_type_question SET subject_id = ?, topic_id = ?, question_image_url =?, correct_answer = ?, total_marks = ?, question_description = ?, solution_image_url = ?, negative_mark = ?, partial_mark = ? WHERE question_id= ?";
...
// delete a particular AITS subjective type question from the AITS subjective type question table
router.delete('/delete-aits-subjective-type-question/:id', function(req, res, next) {
    var id = req.params.id;
    var sql = "DELETE FROM aits_subjective_type_question WHERE question_id=" + mysql.escape(id);
    mysqlConnection.query(sql, function(err, result) {
        if(err)  {
            res.status(202).send({ error: err })
        }
